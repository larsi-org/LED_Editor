...
def find_symmetry(a):
	for i0, p in enumerate(a):
		...
		if f_equal(r, 0.0):
			...
			if f_equal(t, 0.0) or f_equal(t, 30.0):
				i1 = p_find(a, r,  60 + t)
				a[i0]['next'] = i1
				i2 = p_find(a, r, 120 + t)
				a[i1]['next'] = i2
				...
				i9 = p_find(a, r, 300 - t)
				a[i8]['next'] = i9
				i10 = p_find(a, r, 300 + t)
				a[i9]['next'] = i10
				i11 = p_find(a, r, 360 - t)
				a[i10]['next'] = i11
				a[i11]['next'] = i0


def write_layout(name, all_coords):
	leds = [{'x': clean(p['x']), 'y': clean(p['y']), 'r': 0.1} for p in all_coords]
	out = {'leds': leds}

	symmetry = [p['next'] for p in all_coords]
	if any(v != i for i, v in enumerate(symmetry)):
...
def create_hex(n):
	...
	write_layout('hex{}'.format(n), all_coords)
...
def create_circle(n):
	...
	find_symmetry(all_coords)
	write_layout('circle{}'.format(n), all_coords)
